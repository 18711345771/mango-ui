    ...
    <div class="page-container">
        ...
        <el-table :data="tableTreeDdata" stripe size="mini" style="width: 100%;"
            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                    <kt-button icon="fa fa-edit" :label="$t('action.edit')" perms="sys:menu:edit" @click="handleEdit(scope.row)" :size="size"/>
                    <kt-button icon="fa fa-trash" :label="$t('action.delete')" perms="sys:menu:delete" type="danger" @click="handleDelete(scope.row)" :size="size"/>
                </template>
            </el-table-column>
        </el-table>
        <!-- 新增修改界面 -->
    ...
    export default class Menu extends Vue{
        ...
        handleDelete(row:any) {
            this.$confirm("确认删除选中记录吗？", "提示", {
                ...
                (this as any).$api.menu.batchDelete(params).then((res:any) => {
                    this.findTreeData();
            ...
            (this.$refs["dataForm"] as any).validate((valid:boolean) => {
                if (valid) {
                    this.$confirm("确认提交吗？", "提示", {}).then(() => {
                        ...
                        (this as any).$api.menu.save(params).then((res:any) => {
