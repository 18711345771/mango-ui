...
<script lang="ts">
    ...
    export default class Role extends Vue{
        ...
        dialogVisible:boolean= false; // 新增编辑界面是否显示
        ...
        findPage(data:any) {
            ...
            (this as any).$api.role.findPage(this.pageRequest).then((res:any) => {
        ...
        handleDelete(data:any) {
            (this as any).$api.role.batchDelete(data.params).then(data.callback)
        };
        // 显示新增界面
        handleAdd() {
            ...
            (this.$refs.dataForm as any).validate((valid:boolean) => {
                if (valid) {
                    this.$confirm('确认提交吗？', '提示', {}).then(() => {
                        ...
                        (this as any).$api.role.save(params).then((res:any) => {
        ...
        findTreeData() {
            ...
            (this as any).$api.menu.findMenuTree().then((res:any) => {
                this.menuData = res.data;
                this.menuLoading = false
            })
        };
        // 角色选择改变监听
        handleRoleSelectChange(val:any) {
            if(val == null || val.val == null) {
                return
            }
            this.selectRole = val.val
            (this as any).$api.role.findRoleMenus({'roleId':val.val.id}).then((res:any) => {
                this.currentRoleMenus = res.data
                (this.$refs.menuTree as any).setCheckedNodes(res.data)
            })
        };
        // 树节点选择监听
        handleMenuCheckChange(data:any, check:boolean, subCheck:any) {
            if(check) {
                // 节点选中时同步选中父节点
                let parentId:any;
                parentId = data.parentId;
                (this.$refs.menuTree as any).setChecked(parentId, true, false)
            } else {
                // 节点取消选中时同步取消选中子节点
                if(data.children != null) {
                    data.children.forEach((element:any) => {
                        (this.$refs.menuTree as any).setChecked(element.id, false, false)
                    });
                }
            }
        };
        ...
        resetSelection() {
            this.checkAll = false;
            (this.$refs.menuTree as any).setCheckedNodes(this.currentRoleMenus)
        };
        // 全选操作
        handleCheckAll() {
            if(this.checkAll) {
                ...
                (this.$refs.menuTree as any).setCheckedNodes(allMenus);
            } else {
                (this.$refs.menuTree as any).setCheckedNodes([])
            }
        };
        // 递归全选
        checkAllMenu(menuData:any, allMenus:any) {
        ...
        submitAuthForm() {
            ...
            let checkedNodes = (this.$refs.menuTree as any).getCheckedNodes(false, true)
            let roleMenus = []
            for(let i=0, len=checkedNodes.length; i<len; i++) {
                let roleMenu = { roleId:roleId, menuId:checkedNodes[i].id }
                roleMenus.push(roleMenu)
            }
            (this as any).$api.role.saveRoleMenus(roleMenus).then((res:any) => {
                if(res.code == 200) {
                    this.$message({ message: '操作成功', type: 'success' })
                } else {
                    this.$message({message: '操作失败, ' + res.msg, type: 'error'})
