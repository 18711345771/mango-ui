...
<script lang="ts">
    ...
    export default class User extends Vue{
        ...
        findPage(data:any) {
            ...
            (this as any).$api.user.findPage(this.pageRequest).then((res:any) => {
        ...
        // 加载用户角色信息
        findUserRoles() {
            (this as any).$api.role.findAll().then((res:any) => {
                // 加载角色集合
                this.roles = res.data
            })
        };
        ...
        handleDelete(data:any) {
            (this as any).$api.user.batchDelete(data.params).then(data!=null?data.callback:'')
        };
        // 显示新增界面
        handleAdd(){
            ...
            (this.$refs.dataForm as any).validate((valid:boolean) => {
                if (valid) {
                    this.$confirm('确认提交吗？', '提示', {}).then(() => {
                        ...
                        (this as any).$api.user.save(params).then((res:any) => {
        ...
        // 菜单树选中
        deptTreeCurrentChangeHandle (data:any, node:any) {
        ...
        displayFilterColumnsDialog() {
            (this.$refs.tableColumnFilterDialog as any).setDialogVisible(true)
        };
        // 处理表格列过滤显示
        handleFilterColumns(data:any) {
            this.filterColumns = data.filterColumns
            (this.$refs.tableColumnFilterDialog as any).setDialogVisible(false)
        };
        // 处理表格列过滤显示
        initColumns() {
            this.columns = [
