...
<script lang="ts">
    ...
    import {Vue,Provide,Component,Prop} from 'vue-property-decorator'
    @Component({
        components:{
            KtButton
        }
    })
    export default class KtTable extends Vue{
        @Prop() columns!: Array<any>; // 表格列配置
        @Prop()data!: Object; // 表格分页数据
        @Prop()permsEdit!: String;  // 编辑权限标识
        @Prop()permsDelete!: String;  // 删除权限标识
        @Prop()size!: { // 尺寸样式
            type: String,
            default: 'mini'
        };
        ...
        @Prop()highlightCurrentRow!: {  // // 是否高亮当前行
            type: Boolean,
            default: true
        };
        @Prop()showOverflowTooltip!: {  // 是否单行显示
            type: Boolean,
            default: true
        };
        @Prop()showBatchDelete!: {  // 是否显示操作组件
            type: Boolean,
            default: true
        };
        // 分页信息
        ...
        // 换页刷新
        refreshPageRequest(pageNum:any) {
        ...
        // 批量删除
        handleBatchDelete() {
            let ids = this.selections.map((item:any) => item.id).toString()
            this.delete(ids)
        };
        // 删除操作
        delete(ids:any) {
